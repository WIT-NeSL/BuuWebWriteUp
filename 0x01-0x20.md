...
也就是说，pop链：`User->_destruct => FileList->close() => FileList->_call('close') => File->close('/flag.txt') => $results=file_get_contents('flag.txt') => FileList->_destruct() => echo $result`
...
对uname和phone都过滤了，但是地址是直接取出来的，预编译之后在address的地方存在二次注入。
...
`You have an error in your SQL syntax; check the manual that corresponds  to your MySQL server version for the right syntax to use near '\" and  pwd='dfd9312f1b6469fea5482a572e14b446'' at line 1`

是二次注入没跑了。

大概的查询语句：

`select * from user where username="'aaa"\" and pwd='dfd9312f1b6469fea5482a572e14b446'`
